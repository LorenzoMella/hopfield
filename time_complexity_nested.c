...
#define SAVE_FOLDER "time_complexity_plot_nested"
...
int main(int argc, char **argv)
{
    ...
    /* Setting parameters */
    command_line_parser(argc, argv, &max_trials, &trials_per_params,
                        &max_plot_value, &max_plot_points, &pattern_unit_ratio,
                        &coding_level);
    ...
    printf("Testing networks with %d different numbers of units (log scale)\n"
           ...
           "of units)\nMC estimate over %d x %d trials\nCoding level: %g\n\n",
           max_plot_points, min_plot_value, max_plot_value, pattern_unit_ratio,
           max_trials, trials_per_param, coding_level);
    ...
    for (i = 0; i < max_plot_points; ++i) {
        ...
        for (trial = 0; trial < max_; ++trial) {
            ...
            for (h = 0; h < trials_per_params; ++ h) {
                printf("trial (%d,%d) start...\n", trial+1, h+1);
                ...
                hn_fill_rand_pattern(random_initial_state, coding_level,
                                     max_units);
                ...
                clock_start = clock();
                timesteps = hn_test_pattern(net, NULL, max_units, max_units,
                                            utils);
                clock_end = clock();
                ...
                printf("trial (%d,%d) complete. Elapsed CPU time: %.2f secs\n"
                       "Number of updates: %lu\n", trial+1, h+1, secs_diff,
                       timesteps);
        ...
        avg_elapsed_secs[i] /= max_trials*trials_per_param;
        avg_timesteps[i] /= max_trials*trials_per_param;
    ...
    sprintf(savefile_points, "%s/tc_plot_points_n_%d_%.3f.bin", SAVE_FOLDER,
            max_trials, pattern_unit_ratio);
    sprintf(savefile_secs, "%s/tc_plot_secs_n_%d_%.3f.bin", SAVE_FOLDER,
            max_trials, pattern_unit_ratio);
    sprintf(savefile_steps, "%s/tc_plot_steps_n_%d_%.3f.bin", SAVE_FOLDER,
            max_trials, pattern_unit_ratio);
                         ...
                         double *coding_level)
{
    ...
    *coding_level = CODING_LEVEL;
    ...
    switch (argc) {
        default: /* Ignore args beyond argv[6] */
        case 7:
            *coding_level = strtod(argv[6], NULL);
        case 6:
            *pattern_unit_ratio = strtod(argv[5], NULL);
        case 5:
            *max_plot_points = (int)strtol(argv[4], NULL, 10);
        case 4:
            *max_plot_value = (size_t)strtol(argv[3], NULL, 10);
        case 3:
            *trials_per_param = (int)strtol(argv[2], NULL, 10);
        case 2:
